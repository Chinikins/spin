...
func NewSaveCmd(prjOptions *projectOptions) *cobra.Command {
    ...
    cmd.PersistentFlags().StringVarP(&options.projectName, "project-name", "a", "", "name of the project")
    cmd.PersistentFlags().StringVarP(&options.ownerEmail, "owner-email", "", "", "email of the project owner")
...
func saveProject(cmd *cobra.Command, options *saveOptions) error {
    ...
    createProjectTask := map[string]interface{}{
        "job":          []interface{}{map[string]interface{}{"type": "upsertProject", "project": project["project"], "user": "sre@cloudera.com"}},
